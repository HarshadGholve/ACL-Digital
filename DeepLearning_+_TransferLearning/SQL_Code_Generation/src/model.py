...
class TextToSQLTransformer(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        # Assume src and tgt are the source and target sequences
        src_len = src.size(1)  # Length of source sequence
        tgt_len = tgt.size(1)  # Length of target sequence

        # Create src_mask (shape [src_len, src_len])
        src_mask = torch.triu(torch.ones((src_len, src_len), dtype=torch.bool), diagonal=1)
